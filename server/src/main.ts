...
debug.addHandler(debug.createRawConsoleHandler());
let debug: debug.IDefaultLogger;

if (cluster.isMaster) {
    debug = debug.createDefaultLogger('main(master)');
} else {
    debug = debug.createDefaultLogger('main(child)');
}
