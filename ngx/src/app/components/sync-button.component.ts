...
@Component({
    ...
    template: `
        <button [ngClass]="classes" (click)="onClick()" placement="top" #tooltip="ngbTooltip" ngbTooltip triggers="manual"
                [hidden]="config.hidden" [disabled]="config.disabled" [ngStyle]="{'min-width': config.minWidth}">
            <span *ngIf="!config.hideSyncIcon" style="margin-right:10px">
                <fa-icon [icon]="faSync" [pulse]="isBusy">{{config.text}}></fa-icon>
            </span>
...
export class SyncButtonComponent implements OnInit {
    ...
    @ViewChild('tooltip', { static: true }) public tooltip: NgbTooltip;
